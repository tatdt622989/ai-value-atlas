...
export function modelReference(catalog:Catalog,modelId:string,prefs:ValuePreferences,now:Date){
  if(prefs.category!=='all')return boardEntry(catalog,modelId,prefs.category,now);
  ...
  for(const category of CATEGORY_FALLBACK){
    const hit=boardEntry(catalog,modelId,category,now);
    if(hit)return category==='general'?hit:{...hit,cohortSize:Math.min(hit.cohortSize??20,20)};
  }
  return null;
}
export function modelSpeed(catalog:Catalog,modelId:string,now:Date){return catalog.benchmarks.filter(b=>b.modelId===modelId&&b.source==='artificial-analysis'&&b.outputTokensPerSecond!==null&&isFresh(b.freshness,now)).sort((a,b)=>b.measuredAt.localeCompare(a.measuredAt))[0]??null;}
export function valueQuote(catalog:Catalog,offer:Offer,prefs:ValuePreferences,now=new Date()){
  ...
  if(dependencies.some(d=>!isFresh(d.freshness,now)))return {ok:false as const,reason:'價格、額度或基準資料已過期'};
  ...
  if(plan.availability!=='public'||plan.audience!=='individual'||plan.billing.currency!=='USD')return {ok:false as const,reason:'非一般個人可比較的美元方案'};
  ...
  const benchmark=modelReference(catalog,offer.modelId,prefs,now);
  const speed=modelSpeed(catalog,offer.modelId,now);
  ...
  return {ok:true as const,quote:{id:offer.id,offer,plan,model,provider,multiplier,monthlyCost,upfrontCost:upfront,benchmark,
    basis:'official-api-equivalent' as const,evidenceIds,verifiedAt:dependencies.map(d=>d.freshness.verifiedAt).sort()[0],validUntil:dependencies.map(d=>d.freshness.validUntil).sort()[0],
...
export function rankValues(catalog:Catalog,prefs:ValuePreferences,now=new Date(),calibrating=false):ValueResult{
  ...
  for(const offer of catalog.offers){
    ...
    const result=valueQuote(catalog,offer,prefs,now);
  ...
  for(const r of catalog.research){
    if(!r.eligible||r.replacedByOfferId&&catalog.offers.some(o=>o.id===r.replacedByOfferId))continue;
    ...
    if(plan.availability!=='public')continue;
    if(!isFresh(r.freshness,now)||!isFresh(plan.freshness,now)){excluded.push({id:r.id,reason:'原始研究或價格已達複查期限；原值保留於資料庫'});continue;}
    ...
    const benchmark=modelReference(catalog,r.modelId,prefs,now);
    const speed=modelSpeed(catalog,r.modelId,now);
    ...
    const base=prefs.profile==='cached'&&r.basis==='research-calculated'?r.cachedRatio:r.ratio;
    ...
    if(millionTokens===null&&r.basis==='research-estimate'){
      ...
      const refCard=catalog.rateCards.filter(rc=>rc.modelId===r.modelId&&rc.unit==='USD'&&isFresh(rc.freshness,now))
    ...
    quotes.push({id:r.id,plan,model:{...model,name:r.modelLabel},provider,multiplier,monthlyCost,upfrontCost,benchmark,basis:r.basis,research:r,
  ...
  const universe=calibrating?quotes:rankValues(catalog,{...prefs,ranking:'value',budget:null,upfrontBudget:null,allowAnnual:true,providerId:null,query:'',minRank:null,minTokensPerSecond:null},now,true).quotes;
...
export type ValueQuote=Omit<OfficialQuote,'basis'|'offer'|'calculation'> & {
