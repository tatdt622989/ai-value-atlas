...
- **金額欄禁止混用單位**（2026-09-20 更正）：使用者要求「這種不是錢的不要寫」。`每 $1 的價值` 只能用 API 等值金額除以實付現金；requests、tokens、非美元 credits 不能充當這一欄的數字。原始用量留在使用條件／詳情；沒有可採用金額倍率時保留方案及實付價格，明示「僅列價格」，不能硬換算或造出 0×、1×、無限倍。
