...
- `basis: research-estimate` 且 `millionTokens: null` 的條目，查詢時用「multiplier × cash ÷ 該模型官方 USD rateCard 混合價」反推 token 數。模型沒有新鮮的官方 rateCard 時 quote 會是 `missing-comparable-usage`（推薦分數暫不可算；畫面保留原倍率並標示原日期，不顯示空白破折號）。新增依賴此路徑的研究前，先確認模型有官方 rateCard。
