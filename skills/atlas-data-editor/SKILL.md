...
## 2026-09-21 發布更正：有缺就隱藏，先上線

使用者最新明確指示「有缺就隱藏，先上線」。本次發布的可見性與驗收依本節，優先於下方要求全部方案直接顯示、完整覆蓋未滿即停止發布的舊規則。完整資料保留規則繼續適用。

- 主要表格只顯示同時具備有限正倍率、0–100 有限推薦分數的現行情境，維持倍率／分數／長條及短使用條件。兩種排序、分類與搜尋共用 `isCompleteValueQuote`。
- 同方案有完整與不完整情境時，保留完整列，隱藏缺項列。隱藏是呈現篩選，不刪方案、來源、數字或研究，也不捏造 0 分或 1×。
- 發布前後執行 `pnpm atlas coverage <完整快照.json> --visible --strict`，並比對主要 DOM 與符合條件的 quote ID；完整 catalog 另做保留檢查。全部分類與一般／快取檢視不得全空。
- 完整覆蓋及時效缺項仍保存在稽核報告，不能假稱已全部補齊，也不為了清空報告延長來源日期。這些保留資料的缺項不阻擋本次已授權發布。

## 使用者硬性規則：每次更新完整保留，只能多不能少

### 2026-09-20 最新更正：主欄只用倍率與推薦分數

使用者再次明確要求「全部都要用倍率和分數」「全部都要是本來的格式」，並拒絕使用條件長篇說明。以下為主表的現行驗收規則。

- 主表沿用原本倍率／推薦分數與長條的格式；不得以平台額度、美元單價、原生用量替換。這些數字只能是計算輸入或詳情。
- 不能把補了美元值、ID齊全或沒有空白，當成已補完倍率與分數。逐方案驗證倍率、推薦分數及來源計算；缺少者是未完成工作，繼續補資料。
- 主表只談現在：禁止「沿用原資料」「歷史倍率」「不計排行」，也不放前後比較敘事。舊數字、日期與更正理由完整保存在資料及詳情；不能改個標籤就宣稱完成更新。
- 使用條件只顯示短重點；完整公式、來源日期、限制、審核紀錄放詳情，不能把整段稽核說明塞入主表。
- 不得為滿足格式捏造倍率、能力排名或分數；也不得把未採用的歷史數字重新混入現行排名。保留所有方案、原始資料及來源。
- 發布前執行 `pnpm atlas coverage work/catalog-after.json --strict`。此檢查逐一驗證五個用途檢視與兩種快取設定；所有方案須有倍率與分數，所有顯示情境也須有分數。退出碼非零代表未完成，不能把 ID 齊全或貨幣替代值當通過。這是獨立發布驗收，不能以單元測試成功取代；發布後對完整匯出快照重跑。
...
- **金額欄必須做實質換算，禁止空泛標籤**（2026-09-20 再次更正）：使用者拒絕把缺資料全部改成「僅列價格」。保存 API 費率卻沒接上計算是缺陷；先算同模型官方 API 等值並計入費用。明確美元額度及原生 tokens／credits 都是計算依據，單位成本僅能放在詳情，不能取代主表倍率或分數。免費及客製方案若涉及計算定義變更，先釐清定義，不可捏造有限倍率或零分。
...
- **保留歷史不等於重新採用**：原本停用、被取代、ratio=null 而僅存於 reviewNotes 的舊倍率，不得因完整顯示而重新計分、參與比較基準、排序登頂或獲最佳標示。原數字、來源及日期保留在完整資料與詳情，主表只顯示採用中的計算。調整採納資格時逐 ID 記錄理由與來源；不能僅因數字看起來高就任意調低。
...
- **實測與固定費用不能被換算吃掉**：原始 tokens、API 金額及觀測起訖存入 `observedUsage`，不因快取選項重寫；有依據的外推另存係數。一個 5h 樣本不能自動擴成月配額。總 tokens 未提供組成時，可按最低適用 API 價算明示的金額下界，不能捏造 97.5% 或其他實測比例。席位費加用量的方案使用 `fixedMonthlyUSD` 計入分母，並保留年繳及最低席數付款門檻。
...
- **完成標準是主要表格 ID 集合相等**：公開 API 的 quotes 加 plan rows、實際 DOM 主要表格的 `data-plan-id` 集合，皆須等於完整發布版本的所有方案 ID。逐供應商比對；主表只放現行採用的研究情境；歷史倍率、被取代情境及審核敘事保留在資料與詳情，不能放在主表條件欄。只驗資料庫、詳情、搜尋或總數均不合格。
- 每次都以目前完整發布版本為基準，保留所有既有方案、模型、關聯、費率、研究、樣本、權重、原始日期和來源；新方案與新證據累加。禁止只保留本輪查到或仍在核驗期限內的子集合。
- **完整快照必須用管理端匯出**：CLI `atlas export` 或帶既有管理授權的 `GET /api/admin/catalog`。公開 `/api/v1/catalog?includeExpired=true` 仍會清空來源 `excerpt` 與 `locks`，只能驗公開呈現，不能當作完整更新底稿。stage／publish 必須阻擋既有來源原文轉空；發布後比對管理端原文與完整快照。分類 API 驗證使用 `POST /api/v1/value` 的 JSON preferences，並核對回應的 category；GET 不解析查詢參數。
- 到複查期限、抓取失敗、暫缺 benchmark、排程停用，都不是刪除或隱藏既有資料的理由。原始數字與日期完整保存在資料與詳情，再補查最新來源；不得靠延長日期、虛構倍率或評分冒充核驗。
- 每個方案都應呈現已知價格、付款期、額度／限制、來源及已有研究倍率。既有數字不能變成空值或「—」；推薦分數或倍率不能計算時，必須記錄為未完成並繼續核對；不能以另一種數字、空泛標籤或假的 0 分／1× 宣稱補齊。官方未公開固定額度時繼續查社群，不得編造。
...
- 過複查期限時，原始資料、數字及原核對日期保留在資料與詳情，不把舊數字冒充最新。禁止整批隱藏造成資料減少；不要靠延長期限掩蓋更新失敗。
...
- `basis: research-estimate` 且 `millionTokens: null` 的條目，查詢時用「multiplier × cash ÷ 該模型官方 USD rateCard 混合價」反推 token 數。模型沒有新鮮的官方 rateCard 時 quote 的推薦分數狀態會是 `missing-comparable-usage`（代表數字覆蓋仍未完成，需補查可比較用量）。新增依賴此路徑的研究前，先確認模型有官方 rateCard。
- 新模型要能進排行需要對應 `benchmarks`（arena/AA）。模型有條目但無榜單資料 → `abilityPercentile` null → 推薦分數暫不可算；必須持續補查模型的實際能力資料，不能冒用其他模型排名。新增模型時一併核對榜單快照有沒有它。
