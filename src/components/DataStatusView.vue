...
const emit=defineEmits<{detail:[Catalog['plans'][number]]}>();
...
<template><section class="secondary-view"><div class="section-top"><div><h2>{{t('status.title')}}</h2></div><span></span></div><div class="status-summary"><div><strong>{{freshCount}}<small>/{{catalog.plans.length}}</small></strong><span>{{t('status.inDate')}}</span></div><div><strong>{{sourceCount}}</strong><span>{{t('status.sources')}}</span></div><div><strong>{{staleCount}}</strong><span>{{t('status.pendingRecheck')}}</span></div><div><strong>{{publishedDate}}</strong><span>{{t('status.lastUpdated')}}</span></div></div><h2 class="subheading">{{t('status.planSources')}}</h2><div class="source-list"><article v-for="p in sortedPlans" :key="p.id"><div><h3>{{providerName(p.providerId)}} · {{p.name}}</h3><p>{{p.kind==='api'?t('status.apiUsage'):t('status.subFree')}} · {{t('status.verified')}} {{p.freshness.verifiedAt.slice(0,10)}}</p></div><span class="status-label" :class="{expired:!isFresh(p.freshness)}">{{isFresh(p.freshness)?t('status.inDateTag'):t('status.recheckTag')}}</span><button class="row-arrow" :aria-label="t('pending.view',{plan:p.name})" @click="emit('detail',p)">→</button><a :href="catalog.evidence.find(e=>e.id===p.freshness.evidenceIds[0])?.url" target="_blank" rel="noopener noreferrer">{{t('status.viewSource')}}</a></article></div></section></template>
