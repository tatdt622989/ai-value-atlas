...
const researchValues=(rows:ReturnType<typeof pendingValuePlans>[number]['research'])=>rows.map(r=>{
  const value=r.ratio??historicalResearchRatio(r);
  return `${r.modelLabel} ${value===null?t('score.noPublishedUsage'):`${value.toFixed(1)}×${r.ratio===null?` (${t('research.historical')})`:''}`}`;
}).join(' · ');
</script>
<template>
  <details v-if="rows.length" class="unknown-plans pending-plans" :open="open">
    ...
    <article v-for="q in rows" :key="q.plan.id">
      <div><strong>{{td(q.plan.name)}}</strong><small>{{q.modelNames.join(' / ')}}</small><small>{{t('pending.checked',{date:q.plan.freshness.verifiedAt.slice(0,10)})}}</small><small v-if="q.research.length">{{t('pending.research')}}: {{researchValues(q.research)}}</small></div>
