...
import {planMoney,type MoneyComparison} from '../../shared/money';
...
type Row={id:string;plan:Plan;quote:RankedValueQuote|null;benchmark:Benchmark|null;provider:string;models:string;money:MoneyComparison|null};
...
const baseRows=computed<Row[]>(()=>[
 ...quotes.value.map(q=>({id:q.id,plan:q.plan,quote:q,benchmark:q.benchmark,provider:q.provider.name,models:q.model.name,money:null})),
  ...
  <div class="value-table">
   ...
   <template v-if="data">
    <article v-for="(row,i) in rows" :key="row.id" class="value-row value-grid" :data-plan-id="row.plan.id" :data-record-id="row.id" :data-ranking-status="row.quote?.dataStatus==='historical'?'reference':row.quote?'adopted':row.money?.kind" :class="{best:i===0&&row.quote&&row.quote.dataStatus!=='historical'&&(!balanced||displayValue(row.quote)!==null),'plan-only-row':!row.quote}">
     ...
     <div v-else-if="row.money" class="value-cell plan-value monetary-cell"><strong v-if="row.money.amount!==null">{{monetaryAmount(row.money)}}</strong><span v-else>{{moneyLabel(row.money)}}</span><small>{{row.money.amount!==null?moneyLabel(row.money):''}}<template v-if="row.money.unit"> · {{moneyUnit(row.money)}}</template></small></div>
     <div v-else class="value-cell plan-value"><span class="muted">{{t('record.referenceOnly')}}</span></div>
