...
test('Arena keeps only the top 20 of each board',()=>{
 ...
 const r=parseArena(raw,cat,ev);assert.equal(r.benchmarks.length,2);assert.deepEqual(r.belowCutoff,['glm-5.1']);assert.equal(r.benchmarks[0].cohortSize,21);
});
test('AA keeps only each index top 20',()=>{
 ...
 assert.equal(r.benchmarks.length,2);assert.ok(r.benchmarks.every(b=>b.modelId==='glm52'));assert.equal(r.unmatched.length,19);
});
